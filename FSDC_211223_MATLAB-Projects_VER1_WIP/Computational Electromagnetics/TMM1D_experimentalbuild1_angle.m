...
stopPoint = 89;
numPoint = 10000;
THETA = linspace(startPoint, stopPoint , numPoint);

SRC.lam0 = 1.3;                        %free space wavelength
SRC.theta = -89 * degrees;                %elevation angle
...
% DEFINE LAYERS
DEV.UR = [1 1 2.0];                         %array of permeabilities in each layer of unit cell
DEV.ER = [1.5^2 2.1^2 3.0^2];                 %array of permittivities in each layer of unit cell
DEV.L0 = [0.25 0.18, 0.1];                   %array of thickness of each layer of the unit cell
DEV.lam0 = 1;
...
plotLinear(REF, TRN, CON, THETA);

plotdB2(REFdB, TRNdB, CONdB, THETA);
...
function plotL = plotLinear(ref, trn, con, xAxis)
    f1 = ref;
    f2 = trn;
    f3 = con;
    lineWidth = 1.5;
    % OPEN FIGURE WINDOW
    F1 = figure('Color','w', 'Position', [1 42 958 954]); %1922
    ...
    % LABEL AXES
    xlabel('$ \textrm{Angle of Incidence (degrees)} $','Interpreter','latex');
...
function plotdB = plotdB2(ref, trn, con, xAxis)
    f1 = ref;
    f2 = trn;
    f3 = con;
    lineWidth = 1.5;
    % OPEN FIGURE WINDOW
    F1 = figure('Color','w', 'Position', [959 42 958 954]);
    set(F1, 'Name', 'Wavelength Response Graph');
    set(F1, 'NumberTitle', 'off');
    % PLOT DATA
    %subplot(2,2,1);
