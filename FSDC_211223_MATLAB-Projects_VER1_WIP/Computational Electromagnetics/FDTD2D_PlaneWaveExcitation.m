...
% Movie Creation
MAKE_MOVIE = 0;
movie_title = 'FDTD2D_ConvEnergy_longer_simulation.mp4';
if MAKE_MOVIE == 1
...
Nx = round(L/dx) + ceil((BUFZ(1) + BUFZ(2))/(nref*dx)); 
Ny = round(h/dy) + ceil((BUFZ(3) + BUFZ(4))/(nref*dy)); 
...
er_ref = ER2(:, 2*NPML(3) + 1); %One layer in the reflection region's relative permittivity 
er_ref = mean(er_ref(:)); %mean relative permittivity of the reflection grid layer
er_trn = ER2(:, Ny2 - 2*NPML(3) - 1);%One layer in the transmission region's relative permittivity
er_trn = mean(er_trn(:)); %mean relative permittivity of the transmission grid layer
er_src = ER2(:, 2*NPML(3) + 2); %One layer AFTER the reflection region's relative permittivity
er_src = mean(er_src(:)); %mean relative permittivity of the source grid layer

ur_ref = UR2(:, 2*NPML(4) + 1); %One layer in the reflection region's relative permeability 
ur_ref = mean(ur_ref(:)); %mean relative permittivity of the grid layer
ur_trn = UR2(:, Ny2 - 2*NPML(4) - 1);%One layer in the transmission region's relative permeability
ur_trn = mean(ur_trn(:)); %mean relative permittivity of the grid layer
ur_src = UR2(:, 2*NPML(3) + 1); %One layer AFTER the reflection region's relative permittivity
...
Ezsrc_temporal = 1; %exp(-((Time_element - time0)/tau).^2);
omega0 = (2*pi)*(freq);
Ezsrc_planewave = sin(omega0*Time_element);
Ezsrc = Ezsrc_temporal.*Ezsrc_planewave;
...
Hxsrc = sin(omega0*(Time_element +sigma_t));

%Hz Mode

Exsrc_temporal = 1; %exp(-((Time_element - time0)/tau).^2);
Exsrc_planewave = sin(omega0*Time_element);
Exsrc = Exsrc_temporal.*Exsrc_planewave;

A = -sqrt(er_src/ur_src); %-((sigma_yy/(fmax*e0*sqrt(ur_src*er_src)))+sqrt(er_src/ur_src)); -> This is a loss term that needs to be edited
sigma_t = (nsrc*dy)/(2*c0)+dt/2;
%Hzsrc = A.*exp(-((Time_element - time0 + sigma_t)/tau).^2);
Hzsrc = sin(omega0*(Time_element + sigma_t));
...
Ezref = zeros(Nx, NFREQ);
Eztrn = zeros(Nx, NFREQ);
Hzref = zeros(Nx, NFREQ);
Hztrn = zeros(Nx, NFREQ);
EzSRC = zeros(1, NFREQ);
HzSRC = zeros(1, NFREQ);
...
set(fig3, 'Name', 'FDTD-2D Analysis - Source Function');
...
%Ez Mode
    ...
    CEx(:, nysrc-1) = CEx(:, nysrc-1) - Ezsrc(T)/dy;
    ...
    %Inject TF/SF Source into Curl of E
    CHz(:, nysrc) = CHz(:, nysrc) + Hxsrc(T)/dy;
...
%Hz Mode
    ...
    %Inject TF/SF Source into Curl of E
    CEz(:, nysrc-1) = CEz(:, nysrc-1) + Exsrc(T)./dy;
    %Update H Integrations
    IHz = IHz + Hz;
    ...
    %Inject TF/SF Source into Curl of E
    CHx(:, nysrc) = CHx(:, nysrc) - Hzsrc(T)./dy;
    % Update D Integrations
    ...
    for nf = 1: NFREQ
        ...
        Ezref(:,nf) = Ezref(:,nf) + (K(nf)^T.*Ez(:, nyref));
        Eztrn(:,nf) = Eztrn(:,nf) + (K(nf)^T.*Ez(:, nytrn));
        EzSRC(nf) = EzSRC(nf) + (K(nf)^T.*Ezsrc(T));
        
        %Compute Wave Vector Components
        lam0    = c0/FREQ(nf);   % Free Space Wavelength
        k0      = 2*pi/lam0;        % Free Space wave number
        kyinc   = k0*nref;          % Incident wave vector
        m       = [-floor(Nx/2):floor(Nx/2)]'; %Spatial Harmonics Order
        kx      = -2*pi*m/L;         % Wave vector expansion
        kyR     = sqrt((k0*nref)^2 - kx.^2); %ky in reflection region
        kyT     = sqrt((k0*ntrn)^2 - kx.^2); %ky in transmission region
        
        % Compute Reflectance
        ref = Ezref(:, nf)/EzSRC(nf);   %normalize to source
        ref = fftshift(fft(ref))/Nx;    %compute for spatial harmonics
        ref = real(kyR/kyinc).*abs(ref).^2; %compute diffraction efficiencies
        REF(nf) = sum(ref);                 %compute reflectance
        
        % Compute Transmittance
        trn = Eztrn(:, nf)/EzSRC(nf);   %normalize to source
        trn = fftshift(fft(trn))/Nx;    %compute for spatial harmonics
        trn = real((kyT/kyinc).*(ur_ref/ur_trn)).*abs(trn).^2; %compute diffraction efficiencies
        TRN(nf) = sum(trn);                 %compute transmittance
    end
...
%     set(h,'LineWidth',2);
    ...
    subplot(2, 2, [3 4]);
    x_axis = FREQ;
    plot(x_axis, (REF),'-r','LineWidth', 3); hold on;
    plot(x_axis, (TRN),'-b', 'LineWidth', 3);
    plot(x_axis, (CON),'--k', 'LineWidth', 3);
    plot(x_axis, (1 - CON),'--g', 'LineWidth', 3); hold off;
    axis([min(FREQ) max(FREQ) 0 1.25]);
    xlabel('Frequency (GHz)');
    ylabel('Reflectance and Transmittance','Rotation',90);
    title('$Spectral Response - Ez$', 'Interpreter',  'Latex');
