...
erm = -10000; %permittivity of metals
% GRID PARAMETERS
...
ny0 = floor((Ny2 - round(yTdev/dy2))/2)-8; %metal on top
...
ER2(nx1:nx2, ny0:ny1) = erm; %Generate and fill the metal first tooth with the appropriate dielectric values
ER2(nx5:nx6, ny0:ny1) = erm; %Generate and fill the metal second tooth with the appropriate dielectric values
...
k0 = 2*pi./lam0; %Wave vector with Source Frequency
...
% Note that this field shows the Hz Field, z-component - which is the
% E-mode
emode = Ae\bE;
emode = full(emode); %shows the full vector, not the sparse one
emode = reshape(emode,Nx,Ny); %reshape the column vector to matrix form.

% Note that this field shows the Ez Field, z-component - which is the
% H-mode 
hmode = Ah\bH;
hmode = full(hmode); % shows the full vector, not the sparse one
hmode = reshape(hmode, Nx, Ny); %reshape the column vector to matrix form.
...
Eref = emode(:, NPML(3)+10); 
Etrn = emode(:, Ny - NPML(3)-10);

Href = hmode(:, NPML(4) + 10);
Htrn = hmode(:, Ny-NPML(4) -10);
