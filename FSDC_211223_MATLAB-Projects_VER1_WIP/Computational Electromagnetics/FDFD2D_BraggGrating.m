...
d = 0.25*lamd; %grating depth
t = 0.08*lamd; %substrate thickness
...
elseif FREQ == 1
    ...
    for build = 1:GratingPeriod
        b1 = a1 + round(d/dy2) - 1; %location of the tooth base from the top of the grid
        c1 = b1+1; %location of the top surface of the grating substrate from the top of the grid
        d1 = c1 + round(t/dy2) - 1; %location of the bottom surface of the grating from the top of the grid
...
elseif ANGLE == 1
    ...
    ny4 = ny3 + round(t/dy2) - 1; %location of the bottom surface of the grating from the top of the grid
    ny5 = ny4+1;
    ny6 = ny5 + round(d/dy2) - 1;
    ny7 = ny6 + 1;
    ny8 = ny7 + round(t/dy2) - 1;
    
    DEV.ER2 = er1*ones(Nx2,Ny2); %Initiate a grid of all ones times the dielectric of the reflection region (er1).
    DEV.ER2(:, ny1:ny2) = er; %Generate and fill the first tooth with the appropriate dielectric values
    DEV.ER2(:,ny3:ny4) = er_2; %Generate and fill the grating substrate
    DEV.ER2(:, ny5:ny6) = er; %Generate and fill the first tooth with the appropriate dielectric values
    DEV.ER2(:,ny7:ny8) = er_2; %Generate and fill the grating substrate
    DEV.ER2(:,ny4+1:Ny2) = er2; %Fill the transmission region with its dielectric values
