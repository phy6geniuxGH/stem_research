...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SOURCE PARAMETERS
f0 = 24.0 * gigahertz; %operating frequency
...
Nx = floor(L/dx);
Ny = ceil(yTdev/dy) + 2*NPML(3)+ 2.*ceil(BUFZ(1)./(nref*dy));
...
% er_ref = ER2(:, 2*NPML(3)+1:2*NPML(3) + 2);
% er_src = ER2(:, 2*NPML(3)+4:2*NPML(3) + 5);
% er_trn = ER2(:, Ny2 - 2*NPML(3) - 2:Ny2 - 2*NPML(3));
% 
% ur_ref = UR2(:, 2*NPML(4)+1:2*NPML(4) + 2);
% ur_src = UR2(:, 2*NPML(4)+4:2*NPML(4) + 5);
% ur_trn = UR2(:, Ny2 - 2*NPML(4) - 2:Ny2 - 2*NPML(3));
...
%Q1 = reshape(Q, [1 Nx*Ny]);
%spdiags([diagonal_lower -diagonal_0th diagonal_higher], [-NGRID(1)+1 0 1], n,n);
...
Eref = efield(:, NPML(3) + 20);
Esrc = efield(:, NPML(3) + 25);
Etrn = efield(:, Ny - NPML(3) - 20);

Href = hfield(:, NPML(4) + 20);
Hsrc = hfield(:, NPML(4) + 25);
Htrn = hfield(:, Ny - NPML(4) - 20);

% efield(:, NPML(3) + 2) = 10;
% efield(:, NPML(3) + 5) = 10;
% efield(:, Ny - NPML(3) - 2) = 10;
% 
% hfield(:, NPML(4) + 2) = 10;
% hfield(:, NPML(4) + 5) = 10;
% hfield(:, Ny - NPML(4) - 2) = 10;
...
RE_m = ((Sref)'*(Sref)).*(real(ky_ref_m./ur1)./real(kinc(2)/ur1));
RH_m = ((Uref)'*(Uref)).*(real(ky_ref_m./er1)./real(kinc(2)/er1));

TE_m = ((Strn)'*(Strn)).*(real(ky_trn_m./ur2)./real(kinc(2)/ur2));
TH_m = ((Utrn)'*(Utrn)).*(real(ky_trn_m./er2)./real(kinc(2)/er2));
...
% This argument is only needed for periodic boundaries.
    ...
    if NGRID(1) == 1
        ...
        if BC(1) == 0
            ...
            DEX = (1/RES(1))*spdiags([-diagonal_0th diagonal_higher], [0 1], n,n);
        elseif BC(1) == -2
            diagonal_0th = ones(n,1);
            diagonal_higher = ones(n,1);
            diagonal_higher(1:NGRID(1):NGRID(1)*NGRID(2)) = 0;
            ...
            DEX = (1/RES(1))*spdiags([diagonal_lower -diagonal_0th diagonal_higher], [-NGRID(1)+1 0 1], n,n);
        end  
    ...
    if NGRID(2) == 1
       ...
       if BC(2) == 0
            diagonal_0th = ones(n,1);
            diagonal_higher = ones(n,1);
            DEY = (1/RES(2))*spdiags([-diagonal_0th diagonal_higher], [0 NGRID(1)], n,n);
        elseif BC(2) == -2
            diagonal_0th = ones(n,1);
            diagonal_higher = ones(n,1);
            diagonal_lower = zeros(n,1);
            ...
            DEY = (1/RES(2))*spdiags([diagonal_lower -diagonal_0th diagonal_higher], [-(NGRID(1)*NGRID(2)-(NGRID(1)-1))+1 0 NGRID(1)], n,n);
        end  
    end
