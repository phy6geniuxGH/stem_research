...
ur_rl = 1;
er_rl = 3.46;
sigma_yy = 5*10^-4; %Testing Conductivity values.

L = 100;
h = 100;
th = 30.48*centimeters;
d = 1.6779*centimeters;
...
% Movie Creation
MAKE_MOVIE = 0;
movie_title = 'FDTD1D_IncorporatingLoss_AbsorptionPlot.mp4';
if MAKE_MOVIE == 1
...
nz2 = nz1 + round(d/dz) - 1;
nz3 = nz2 + 1;
nz4 = nz3 + round(th/dz) - 1;
nz5 = nz4 + 1;
nz6 = nz5 + round(d/dz) - 1;
ER = ones(1, Nz);
ER(nz1:nz2) = er_rl;
ER(nz3:nz4) = er;
ER(nz5:nz6) = er_rl;
...
mEy1 = abs((2*e0.*ER - dt*sigma_yy)./(2*e0.*ER + dt*sigma_yy));
mEy2 =  (2*e0*c0*dt)./(2*e0.*ER + dt*sigma_yy);
mHx = (c0*dt)./UR;
...
Tprop = (nmax*Nz*dz)/c0;
Ttotal = 15*tau + 7*Tprop;
STEPS = ceil(Ttotal/dt);

Time_element = [0:STEPS-1]*dt;
Eysrc  = exp(-((Time_element - time0)/tau).^2);
A = -sqrt(er_src/ur_src); %-((sigma_yy/(fmax*e0*sqrt(ur_src*er_src)))+sqrt(er_src/ur_src));
...
for T = 1:counter:STEPS
    ...
    E2=E1; E1 = Ey(Nz);
    Ey(1) = mEy1(1)*Ey(1) + mEy2(1)*(Hx(1) - H2)/dz;
    for nz = 2:Nz
        Ey(nz) = mEy1(1)*Ey(nz) + mEy2(nz)*(Hx(nz) - Hx(nz - 1))/dz;
    end
    % E-Field Source
    Ey(nzsrc) = Ey(nzsrc) - (mEy2(nzsrc)/dz)*Hxsrc(T);
    ...
    x_fill2 = [nz3 nz4 nz4 nz3 nz3]*dz;
    x_fill3 = [nz5 nz6 nz6 nz5 nz5]*dz;
    y_fill = [-3 -3 3 3 -3];
    fill(x_fill1,y_fill,'c'); hold on;
    fill(x_fill2,y_fill,'m');
    fill(x_fill3,y_fill,'c');
