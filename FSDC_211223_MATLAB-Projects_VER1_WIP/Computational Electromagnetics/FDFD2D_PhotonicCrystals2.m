...
NPx = 10; %number of lattice periods to draw;
...
DEV.NPML = [0 0 50 50]; %size of PML at top and bottom of grid
...
elseif FREQ == 1
    ...
    nL = sqrt(ur_1*er_1); % refractive index of the hole
    nH = sqrt(ur_2*er_2); % refractive index of the cladding
    n_ref = sqrt(ur1*er1); % refractive index of the reflection region
    n_trn = sqrt(ur2*er2); % refractive index of the transmission region

    % Consider the Wavelength for resolving the resolution of the simulation

    n_max = max([nL nH n_ref n_trn]);
    lambda_min = min(LAMBDA)/n_max;
    delta_gammaX = lambda_min/NRES(1);
    delta_gammaY = lambda_min/NRES(1);

    % Consider the Mechanical Features for optimizing the simulaiton
    mech_featX = [r]; %array of mechanical features
    mech_featY = [r];
    d_minX = min(mech_featX)/4;
    d_minY = min(mech_featY)/4;
    ...
    UC = (X.^2 + Y.^2) <= r^2;
    UC = er_2*(UC == 0) + er_1*(UC == 1);

    % Photonic Crystal

    DEV.ER2 = er1*ones(Nx2, Ny2);
    ny1 = round(2*DEV.NPML(3) + (3/2)*round(BUFZ/dy2));
    for np = NP1+1:NP1+NP2
        nya = ny1 + (np - 1)*Ny2_uc;
        nyb = nya + Ny2_uc - 1;
        DEV.ER2(:,nya:nyb) = UC;
    end
    DEV.ER2(:,nyb+1:Ny2) = er_1;
    ...
    UC2 = (X.^2 + Y.^2) <= r^2;
    UC2 = ur_2*(UC2 == 1) + ur_1*(UC2 == 0);
        for np = NP1+1:NP1+NP2
        nya = ny1 + (np - 1)*Ny2_uc;
        nyb = nya + Ny2_uc - 1;
        DEV.UR2(:,nya:nyb) = UC2;
        end
    DEV.UR2(:,nyb+1:Ny2) = ur_1;
    P2 = zeros(Nx2*NPx, Ny2); 
    for npx = 1:NPx
        nx1 = (npx-1)*Nx2 + 1;
        nx2 = nx1 + Nx2 - 1;
        P2(nx1:nx2, :) = DEV.UR2;
    ...
    h = imagesc(xa3/micrometers,ya2/micrometers, P2.',[1 10]);
    h2 = get(h, 'Parent');
    set(h2,'FontSize',10,'LineWidth',0.5);
    xlabel('$x$','Interpreter','LaTex');
    ylabel('$y$','Interpreter','Latex','Rotation',0,'HorizontalAlignment','right');
    ...
    g = imagesc(xa3/micrometers,ya2/micrometers, P.');
    g2 = get(g, 'Parent');
    set(g2,'FontSize',10,'LineWidth',0.5);
    xlabel('$x$','Interpreter','LaTex');
    ylabel('$y$','Interpreter','Latex','Rotation',0,'HorizontalAlignment','right');
    ...
    for nlam = 1:NLAM
        ...
        if etime(clock, t0) > dT
            ...
            for npx = 1 : NPx
                nx1 = (npx-1)*Nx + 1;
                nx2 = nx1 + Nx - 1;
                P(nx1:nx2,:) = Field*exp(1i*kxinc*Nx*dx*npx-1);
            end
            % show graphics
            subplot(1,numgraph,3);
            
            h = imagesc(xa3/micrometers,ya2/micrometers, real(P).');
            h2 = get(h, 'Parent');
            set(h2,'FontSize',10,'LineWidth',0.5);
