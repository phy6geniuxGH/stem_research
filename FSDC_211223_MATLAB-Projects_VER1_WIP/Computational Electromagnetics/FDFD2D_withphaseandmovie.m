...
SRC.scalar_pos = 0.5; % 0 < x <= 1 position of the beam.
...
%lamd = 1;
L = 5000*nanometers; %length of grating
width = 0.5*L;
height = 134*nanometers;  %tooth height
subsheight = 300*nanometers;
% h = 0.85*LAMBDA(1);  %tooth height
ur = 1; %relative permeability of grating
er = 1.38^2; %dielectric constant of grating
ur_2 = 1.0;
er_2 = 1.43^2;
ur_3 = 1.0;
er_3 = 1.43^2;
...
% Movie Creation
MAKE_MOVIE = 0;
movie_title = 'Phase Animation.mp4';
if MAKE_MOVIE == 1
...
elseif FREQ == 1
    ...
    Nx = 2*round(Nx/2)+1; %physical size along x-axis
    Ny = round(sum(mech_featY)/dy)+ DEV.NPML(3)+ DEV.NPML(4)+ 2.*ceil((2*BUFZ)/(n_ref*dy)); %physical size along y-axis
    Ny = 2*round(Ny/2)+1;
    ...
    pos_y1 = DEV.NPML(3) + 2*round(BUFZ/dy2);
    pos_y1 = 2*round(pos_y1/2)+1;
    pos_y2 = pos_y1 + round(height/dy2) - 1;
    pos_y3 = pos_y2 + 1;
    ...
    DEV.ER2 = er1*ones(Nx2,Ny2);
    DEV.ER2(pos_x1:pos_x7,pos_y1:pos_y2) = er_2;
    DEV.ER2(pos_x3:pos_x4,pos_y1:pos_y2) = er; 
    DEV.ER2(pos_x1:pos_x7, pos_y3:pos_y4) = er_3;
    DEV.ER2(:, pos_y4+1:Ny2) = er2;
    
    DEV.UR2 = ur1*ones(Nx2,Ny2); 
    DEV.UR2(pos_x1:pos_x3,pos_y1:pos_y2) = ur_2;
    DEV.UR2(pos_x2:pos_x4,pos_y1:pos_y2) = ur; 
    DEV.UR2(pos_x1:pos_x3, pos_y3:pos_y4) = ur_3;
