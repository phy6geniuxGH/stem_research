...
% SOURCE PARAMETERS
%f0 = [0.299:0.01:0.599]* gigahertz; %operating frequency
...
% GRATING PARAMETERS
%fd = 0.299 * gigahertz; %design frequency
%lamd = c0/fd; %design wavelength
lamd = 1;
L = 0.25*lamd; %period of grating
thickness1 = 0.25*lamd; %grating depth
thickness2 = 0.18*lamd; %substrate thickness
thickness3 = 0.1*lamd;
ur = 1.0; %relative permeability of grating
er = 1.5^2; %dielectric constant of grating
ur_2 = 1.0;
er_2 = 2.1^2;
ur_3 = 2.0;
er_3 = 3.0^2;
...
if CONV == 1
    ...
    for index = 1:length(NRES)
        ...
        ny1 = 1 + floor((Ny2 - round(yTdev/dy2))/2); %location of the top surface of the grating from the top of the grid
        ...
        DEV.ER2(:,ny1:ny2) = er; %Generate and fill the first tooth with the appropriate dielectric values
        DEV.ER2(:,ny3:ny4) = er_2; %Generate and fill the grating substrate
...
elseif FREQ == 1
    ...
    mech_featY = [thickness1, thickness2, thickness3];
    ...
    % Resolving Critical Dimensions through "Snapping"
    xTdev = sum(mech_featX); %the critical dimension here is the duty cycle of the grating
    yTdev = sum(mech_featY); %the height and the substrate thickness
    ...
    DEV.ER2 = er1*ones(Nx2,Ny2); %Initiate a grid of all ones times the dielectric of the reflection region (er1).
    a1 = ceil((Ny2 - round(yTdev/dy2))/2); %location of the top surface of the grating from the top of the grid
    for build = 1:GratingPeriod
        b1 = a1 + round(thickness1/dy2) - 1; %location of the tooth base from the top of the grid
        c1 = b1+1; %location of the top surface of the grating substrate from the top of the grid
        d1 = c1 + round(thickness2/dy2) - 1; %location of the bottom surface of the grating from the top of the grid
        e1 = d1 + 1;
        f1 = e1 + round(thickness3/dy2) - 1;
        DEV.ER2(:,a1:b1) = er; %Generate and fill the first tooth with the appropriate dielectric values
        DEV.ER2(:,c1:d1) = er_2; %Generate and fill the grating substrate
        DEV.ER2(:,e1:f1) = er_3;
        a1 = f1+1;
    end
    
    DEV.ER2(:,a1:Ny2) = er2; %Fill the transmission region with its dielectric values
    
    DEV.UR2 = ur1*ones(Nx2,Ny2); %Since the relative permeability remains 1 (non-magnetic), fill the entire grid with ur1 = 1
    a2 = ceil((Ny2 - round(yTdev/dy2))/2);
    for build = 1:GratingPeriod
        b2 = a2 + round(thickness1/dy2) - 1; %location of the tooth base from the top of the grid
        c2 = b2+1; %location of the top surface of the grating substrate from the top of the grid
        d2 = c2 + round(thickness2/dy2) - 1; %location of the bottom surface of the grating from the top of the grid
        e2 = d2 + 1;
        f2 = e2 + round(thickness3/dy2) - 1;
        DEV.UR2(:,a2:b2) = ur; %Generate and fill the first tooth with the appropriate dielectric values
        DEV.UR2(:,c2:d2) = ur_2; %Generate and fill the grating substrate
        DEV.UR2(:,e2:f2) = ur_3;
        a2 = f2+1;
    end
    DEV.UR2(:,a2:Ny2) = ur2;
...
elseif ANGLE == 1
    ...
    DEV.UR2(:,a2:Ny2) = ur2; %Since the relative permeability remains 1 (non-magnetic), fill the entire grid with ur1 = 1
    ...
    for tht = 1:THT
        ...
        ylabel('$y$','Interpreter','Latex','Rotation',0,'HorizontalAlignment','right');
        mode = convertCharsToStrings(SRC.MODE);
        title([mode+ '-mode at ' + num2str(c0/LAMBDA(1)/gigahertz) + ' GHz', 'Re\{F\}']);
        axis([-1 +1 -1 +1]);
        axis equal tight;
        colorbar;
        %colormap(jet(1024));
        ...
        xlabel('$x$','Interpreter','LaTex');
        ylabel('$y$','Interpreter','Latex','Rotation',0,'HorizontalAlignment','right');
        title([mode+ '-mode at ' + num2str(c0/LAMBDA(1)/gigahertz) + ' GHz' , 'Im\{F\}']);
        shading interp;
        axis([-1 +1 -1 +1]);
        axis equal tight;
        colorbar;
