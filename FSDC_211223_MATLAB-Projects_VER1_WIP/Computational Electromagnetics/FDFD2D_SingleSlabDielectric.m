...
% The code was built from scra tch with just the help of CEM Lectures.
...
% SOURCE PARAMETERS
f0 = [0.1:0.01:2.4]* gigahertz; %operating frequency
LAMBDA = c0./f0; %operating wavelength in free space
SRC.theta = [0] * degrees; %angle of incidence
THETA = SRC.theta;
SRC.MODE = 'E'; %electqromagnetic mode, 'E' or 'H'
% GRATING PARAMETERS
fd = 1.0 * gigahertz; %design frequency
...
ur = 2; %relative permeability of grating
er = 6; %dielectric constant of grating
% EXTERNAL MATERIALS
...
BUFZ = min(LAMBDA); %spacer region above and below grating
DEV.NPML = [0 0 40 40]; %size of PML at top and bottom of grid
...
if CONV == 1
    ...
    for index = 1:length(NRES)
        ...
        dx = min(mech_featX)/Mx; %the adjusted dx
        dy = min(mech_featY)/My; %the adjusted dy

        Nx = round(L/dx);
        Nx = 2*round(Nx/2)+1; %physical size along x-axis
        Ny = round(sum(mech_featY)/dy)+ 2.*DEV.NPML(3)+ 2.*ceil(max(BUFZ)/(n_ref*dy)); %physical size along y-axis
        ...
        nx1 = floor((Nx2 - round(sum(mech_featX)/dx2))/2); %location of the left side of the first tooth from the left side of the grid
        ...
        ny1 = floor((Ny2 - round(sum(mech_featY)/dy2))/2); %location of the top surface of the grating from the top of the grid
...
%             title('SPECTRAL RESPONSE');
    ...
    % For Convergence Test
        ...
        plot(NRES(1:index),REF2(1:index),'ro'); hold on;
        plot(NRES(1:index),TRN2(1:index),'bo'); 
        plot(NRES(1:index),CON2(1:index),'k--'); hold off;
...
elseif FREQ == 1
    ...
    Nx = round(L/dx);
    Nx = 2*round(Nx/2)+1; %physical size along x-axis
    Ny = round(sum(mech_featY)/dy)+ 2.*DEV.NPML(3)+ 2.*ceil(BUFZ/(n_ref*dy)); %physical size along y-axis
    ...
    nx1 = ceil((Nx2 - round(sum(mech_featX)/dx2))/2); %location of the left side of the first tooth from the left side of the grid
    nx2 = nx1 + round(x1/dx2) - 1; %location of the right side of the first tooth from the left side of the grid
    nx3 = nx2+1; %location of the left side of the gap from the left side of the grid
    nx4 = nx3 + round(x2/dx2) - 1; %location of the right side of the gap from the left side of the grid
    nx5 = nx4+1; %location of the left side of the second tooth from the left side of the grid
    ...
    ny1 = ceil((Ny2 - round(sum(mech_featY)/dy2))/2); %location of the top surface of the grating from the top of the grid
    ny2 = ny1 + round(d/dy2) - 1; %location of the tooth base from the top of the grid
    ny3 = ny2+1; %location of the top surface of the grating substrate from the top of the grid
    ny4 = ny3 + round(t/dy2) - 1; %location of the bottom surface of the grating from the top of the grid
