...
% SOURCE PARAMETERS
f0 = [4:0.1:12]* gigahertz; %operating frequency
SRC.lam0 = c0/f0; %operating wavelength in free space
SRC.theta = 15.0 * degrees; %angle of incidence
...
BUFZ = 2*SRC.lam0 * [1 1]; %spacer region above and below grating
...
Ny = round(yTdev/dy)+ 2.*DEV.NPML(3)+ 2.*ceil(2*SRC.lam0/(n_ref*dy)); %physical size along y-axis
...
disp(['REF(' num2str(NRES(1)) ') = ' num2str(REF)]);
disp(['TRN(' num2str(NRES(1)) ') = ' num2str(TRN)]);
disp(['CON(' num2str(NRES(1)) ') = ' num2str(CON)]);
...
% subplot(1,numgraph,3:6);
% plot(NRES(1:index),REF(1:index),'r-o'); hold on;
% plot(NRES(1:index),TRN(1:index),'b-o'); 
% plot(NRES(1:index),CON(1:index),'k--o'); hold off;
