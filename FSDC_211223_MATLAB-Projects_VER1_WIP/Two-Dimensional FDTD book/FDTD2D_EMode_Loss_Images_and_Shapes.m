...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% SELECT IF "IMAGE" OR "SHAPE" IMPORT
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
image_import = 0;
shape_import = 0;

% Name of the shape dataset
%import_the_shape = 'pixelShapes.mat';
import_the_shape = 'manyShapes_150_per_type.mat';
%import_the_shape = 'pixel_binary_figure.mat';

% Individual, Collection, or pixel-binary import
individual_import = 0;
collection_import = 0;
pixel_binary_import = 0;

% Save the optical responses
save_the_optical_responses = 0;
Filename = 'New_set_100_per_type';

% Create validation set
val_set = 0;
num_validation = 100;

% If dataset is in HDF5 file
hdf5_file_format = 1;
hdf5_file_filename = 'Shapes_2500_per_type_hdf5.h5';

% List of samples to be tested 
list_of_samples = [55340, 25247, 49673, 27562, 2653, 16968, 33506, 31845, 26537, 51367, 54385, 19877, 31234, 23465, 38232, 14315, 33075, 9127, 18470, 9158, 49532, 6214, 40525, 52389, 16417, 34902, 46214, 53098, 39446, 9631, 20325, 6472, 47830, 4832, 55736, 44825, 21639, 30942, 36687, 6599, 23186, 28453, 20722, 40035, 41970, 13400, 36210, 31261, 29012, 34167];
...
if image_import
    ...
    addpath 'D:\Research\Aseprite Designs\Metagrating Design'
    imagefiles = dir('D:\Research\Aseprite Designs\Metagrating Design\*.png');      
...
if shape_import
    ...
    if individual_import
        nfiles = length(ShapeType);
    elseif collection_import
        nfiles = length(ShapeCollection);
    else 
        nfiles = length(pixel_binary_Collection);
    end
    
    if collection_import && val_set
        Validation_set = struct('DevER', ones(Nx, Ny, 1));
        rand_samples = randi([1, size(ShapeCollection,2)], 1, num_validation);
        for array_index = 1:num_validation
           Validation_set(array_index).DevER = ShapeCollection(rand_samples(array_index)).DevER;
        end
        nfiles = length(Validation_set);
    end
...
if hdf5_file_format
    ...
    hdf5_data = struct(f0,v0);
    
    for index = 1:length(list_of_samples)
        hdf5_data(index).DevER = h5read(hdf5_file_filename,...
                                        strcat('/material/DevER/',num2str(list_of_samples(index))));
    end
    nfiles = length(hdf5_data);
    load_time2 = clock;
    load_time = etime(load_time2, load_time1);
    disp(['Elapsed time is ' num2str(load_time) ' seconds.']);
    disp(['Elapsed time is ' num2str(load_time/60) ' minutes.']);
...
for imageIndex = 1:nfiles
    ...
    if shape_import
        ...
        if individual_import 
            lastimage = ShapeType(imageIndex).DevER;
        elseif collection_import
            lastimage = ShapeCollection(imageIndex).DevER;
        else
            lastimage = pixel_binary_Collection(imageIndex).DevER;
        end
        
        if collection_import && val_set
            lastimage = Validation_set(imageIndex).DevER;
        end
        ...
        %lastimage(lastimage > 0) = ceil(lastimage(lastimage > 0));
        ERzz(:,ny1:ny2)  = lastimage;
        SIGzz(:,ny1:ny2) = lastimage;
    ...
    if hdf5_file_format
        lastimage = hdf5_data(imageIndex).DevER;
    ...
    disp(['Structure ' num2str(imageIndex) ' is being simulated. ']);
    ...
    imageER_zz      = ERzz(:,ny1:ny2);
    imageSIG_zz     = SIGzz(:,ny1:ny2);
    ...
    % STOP TIMER
    t_2 = clock;
    time_per_sample = etime(t_2,t_1);
    disp(['Computation time per sample is ' num2str(time_per_sample) ...
        ' seconds.']);
end
...
if save_the_optical_responses
        % SAVE THE SHAPES
        save(Filename,'Spectrum','-v7.3','-nocompression');
