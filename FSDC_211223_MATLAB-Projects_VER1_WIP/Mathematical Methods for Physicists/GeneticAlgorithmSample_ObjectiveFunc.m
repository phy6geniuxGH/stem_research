...
YY = XX;
[YYY,XXX] = meshgrid(XX);
ZZZ = 3.*(1-XXX).^2.*exp(-XXX.^2 - (YYY+1).^2) - cos(XXX) + cos(YYY);
...
for i = 1:tg
    ...
    title('$ \textrm{Genetic Algorithm - Maximization Problem} $','Interpreter','latex','FontSize', 18);
    axis([0 tg 0 10]);  
    xlabel('$ \textrm{Number of Generations} $','Interpreter','latex');
    ylabel('$ \textrm{Maximum Value} $','Interpreter','latex');
    h = legend('Average Value','Maximum','Location','NorthEast');
    set(h,'LineWidth',2);
    hold off;
    grid on;
    drawnow;
...
H = zeros(1,x1);
    for i = 1:x1
        ...
        y = -3+B*(3-(-3))/(2^(y1/2) - 1);
        H(1,i) = 3*(1-x)^2.*exp(-x^2 - (y+1)^2) - cos(x) + cos(y); %Objective Function
    end
eval = H;
end
